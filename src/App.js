...
function App() {
  const [signupInput, setSignInput] = useState({
    ...
    password: '',
    confirmPassword: '',
  }); // save values in state so we can add an error that shows up in case state values are null
  ...
  const handleChange = (e) => {
    setSignInput({
      ...signupInput,
      [e.target.name]: e.target.value, // this! to make it reusable for all inputs
    });
  };
    ...
    <div className="container my-5 form-container">
      <form>
        ...
        <div className="mb-3">
          ...
          {/* password doesn't have implictir role */}
          <input
            type="password"
            id="password"
            name="password"
